...
existing_backups = [
    x
    for x in backup_folder_path.iterdir()
    if x.is_file() and x.suffix == ".zip" and x.name.startswith("backup-")
]

sorted_backup = list(sorted(existing_backups, key=lambda f: f.name))
...
backup_file_name = (
    f'backup_{datetime.now().strftime("%Y%m%d%H%M%S")}_{source_folder_path.name}.zip'
)

zip_file = zipfile.ZipFile(str(backup_folder_path / backup_file_name), mode="w")
