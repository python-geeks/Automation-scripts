...
def get_info(sign, day):
    """This function makes the API call to get the result of the desired zodiac sign for the desired day."""
    ...
    if robj.status_code == 200:
        res = f'''We see that you are {sign.capitalize()}. Your horoscope for the date {return_object['current_date']} says that : '{return_object["description"]}'\n
...
def bad_input(container, code):
    """This function is used to handle bad user input and gives the user a choice to either QUIT the game or enter a valid input."""
...
def main():
    """This function displays the complete menu to the user and asks for user input. It also invokes bad_input() function in case of an invalid entry by the user.
    It then invokes the get_info() function to collect the required result and print it."""
    ...
    zodiac = {'1' : 'aries', '2' : 'taurus', '3' : 'gemini', '4' : 'cancer', '5' : 'leo',
    ...
    while True:
        ...
        print(f'\n\nWe have an astro prediction for you!\n\n" {get_info(zodiac[zodiac_code], day[day_code])} "\n')
