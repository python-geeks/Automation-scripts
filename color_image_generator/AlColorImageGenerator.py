import os
from tkinter import (BOTH, END, SUNKEN, Button, Frame, Label, Text, Tk, X,
                     filedialog, font)
    ...
    def __init__(self):
        ...
        def openImage():
            ...
            filename = filedialog.askopenfilename(
                filetypes=[
                    ("Image Files", "*.jpg *.jpeg " "*.bmp *.png " "*.webp *.tiff")
                ]
            )
        ...
        def convert():
            net = cv2.dnn.readNetFromCaffe(
                cwd + "\\AlColorImageGenerator\\mod"
                "el\\colorization_deploy_v2.prototx"
                "t",
                cwd + "\\AlColorImageGenerator"
            ...
            net.getLayer(conv8).blobs = [np.full([1, 313], 2.606, dtype="float" "32")]
        ...
        icon = Image.open(
            os.path.join(cwd + "\\UI\\icons", "alcolorimagegenerator.ico")
        )
