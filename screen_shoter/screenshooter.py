...
class TwoClicksScreenShot:
    ...
    def capture():
        ...
        if TwoClicksScreenShot.defined:
            TwoClicksScreenShot.screenShot = pyautogui.screenshot(
                region=(
                    TwoClicksScreenShot.pCords[0],
                    TwoClicksScreenShot.pCords[1],
                    TwoClicksScreenShot.pCords[2],
                    TwoClicksScreenShot.pCords[3],
                )
            )
            TwoClicksScreenShot.screenShot = cv2.cvtColor(
                np.array(TwoClicksScreenShot.screenShot), cv2.COLOR_BGR2RGB
            )
            return True
    ...
    def output():
        cv2.imwrite(
            TwoClicksScreenShot.filename + ".png", TwoClicksScreenShot.screenShot
        )
