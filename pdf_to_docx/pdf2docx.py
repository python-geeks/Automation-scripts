...
def cloud_convert(app_sid, app_key):
    ...
    try:
        ...
        filename = input(
            "Please enter the complete and correct file path of the required pdf: "
        )
        ...
        print(
            "Exception when calling get_supported_conversion_types: {0}".format(
                e.message
            )
        )
...
if __name__ == "__main__":
    ...
    choice = input(
        "Press 1 for basic file conversion, 2 for advanced file conversion(GroupDocs account required)"
    )
