...
for j in range(1, 11):
    source = requests.get(f"https://1lib.in/s/data%20science?page={j}").text
    ...
    for i in books:
        ...
        # year
        try:
            Year.append(i.find("div", class_="property_year").text.strip()[6:10])
        except Exception:
            Year.append("nan")
        # publisher
        try:
            Publisher.append(i.find("div", attrs={"title": "Publisher"}).text.strip())
        except Exception:
