...
duration_of_video = subprocess.run(["youtube-dl", "--no-check-certificate", "--get-duration", youtube_link], stdout=subprocess.PIPE, text=True)
...
download_link = subprocess.run(["youtube-dl", "--no-check-certificate", "-f", "22", "--get-url", youtube_link], stdout=subprocess.PIPE, text=True)
link = download_link.stdout

#downloading the video from start_time to end_time
subprocess.run(["ffmpeg","-ss",str(start_time),"-i",link,"-t",str(duration) ,"-c:v","copy","-c:a","copy",Output_file_name])
