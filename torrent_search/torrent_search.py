...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("query", metavar="QUERY", type=str, help="the search term")
    parser.add_argument(
        "-s",
        "--seeder-limit",
        dest="seeder_limit",
...
async def send_query(url: str, session: ClientSession, headers: dict = None) -> str:
    response = await session.request(method="GET", url=url, headers=headers)
    response.raise_for_status()
    return await response.text(errors="ignore")


async def parse_thepiratebay(args: argparse.Namespace, session: ClientSession) -> None:
    page = 1

    while True:
        try:
            html = await send_query(
                urlList["thepiratebay"].format_map(
                    {"query": quote(args.query), "page": page}
                ),
                session,
            )
        ...
        for row in rows:
            ...
            category = category_list.get(category.lower(), category_list.get("unknown"))
            title = sub(
...
async def parse_1337x(args: argparse.Namespace, session: ClientSession) -> None:
    category_mappings = {
    ...
    while True:
        try:
            html = await send_query(
                urlList["1337x"].format_map({"query": quote(args.query), "page": page}),
                session,
            )
        except (ClientError, HttpProcessingError):
            break
        ...
        for row in rows:
            ...
            category = category_mappings.get(category, category_list.get("unknown"))
            ...
            results[category].append(
                to_object(title=title, url=url, seeder=seeder, leecher=leecher)
            )
...
async def run(args):
    task_list = [parse_thepiratebay, parse_1337x, parse_torlock]
