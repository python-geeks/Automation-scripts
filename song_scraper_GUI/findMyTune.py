...
from tkinter import (BOTH, CENTER, DISABLED, LEFT, NE, NORMAL, RIGHT, TOP, YES,
                     Button, Entry, Frame, Label, PhotoImage, Tk, Toplevel, W,
                     ttk)

from mp3Clan_top_web_scraper import downloadMySong, findMySong
...
class SongGUI:
    def __init__(self, root):
        ...
        self.help_button = Button(
            self.root, image=help_button_img, command=self.helpFrame, border=0
        )
        ...
        # creating heading
        heading = Label(
            self.input_frame, text="Find My Tune", font=("Helvetica", 30), fg="black"
        )
        heading.pack(pady=20, side=TOP)

        # creating search bar
        self.search_box_input = Entry(
            self.input_frame, font=("Helvetica", 20), textvariable=self.searchname
        )
        ...
        self.search_button = Button(
            self.input_frame, image=search_button_img, border=0, command=self.searchSong
        )
    ...
    def helpFrame(self):
        ...
        heading = Label(
            self.help_window, font=("Helvetica 20 underline"), text="HELP WINDOW"
        )
        ...
        Label(
            self.help_window, font=("Helvetica 16 underline bold"), text="INSTRUCTIONS"
        ...
        Label(self.help_window, font=("Helvetica 16 underline"), text="NOTE").pack(
    ...
    def searchSong(self):
        ...
        if self.songname:
            ...
            if self.songlist:
                ...
                self.results.column("Duration", stretch=YES, anchor=W, width=80)
                ...
                tkinter.messagebox.showerror(
                    "No Song Foumd", "Could'nt find your song, please try again"
                )
                self.search_button["state"] = NORMAL

        else:
            tkinter.messagebox.showerror("Search box Empty", "Please enter song name")
            self.search_button["state"] = NORMAL
        ...
        if songNumber:
            ...
            if filename:
                ...
                tkinter.messagebox.showinfo(
                    "Song Downloaded Successfully", "Your song is downloaded."
                )
            else:
                tkinter.messagebox.showerror(
                    "Song Not Downloaded",
                ...
                filename = [f for f in os.listdir() if f.endswith(".crdownload")]
                if filename:
    ...
    def refreshFrame(self):
        ...
        try:
            ...
            tkinter.messagebox.showinfo("Already Refreshed", "window already refreshed")
