...
import tkinter.ttk
from tkinter import HORIZONTAL, Label, PhotoImage, StringVar, Tk, filedialog
...
text_intro = Label(
    main, text="Convert anything to a QR Code! Select what you want to convert:"
)
...
def textbox():
    Label(main, text="Enter your text: ").grid(
        row=0, column=0, columnspan=4, padx=10, pady=10
    )
    entryfield = tkinter.Entry(
        main, textvariable=entered_text, width=50, borderwidth=5, bg="#FEFFE2"
    )
    entryfield.grid(row=4, column=1, pady=20)
    Button(main, text="Submit", command=lambda: [submit(entered_text.get())]).grid(
        row=5, column=2, columnspan=4
    )
...
def uploadFiles():
    ...
    for i in range(50):
        ...
        time.sleep(0.02)
    progbar.destroy()
    tkinter.Label(main, text="Upload Successful!", fg="green").grid(
        row=5, columnspan=3, pady=10
    )


my_btn3 = Button(main, text="URL", command=lambda: [clear(), textbox()]).grid(
    row=3, column=2
)
