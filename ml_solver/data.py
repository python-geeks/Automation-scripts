...
from sklearn.linear_model import (ARDRegression, BayesianRidge, ElasticNet,
                                  ...
                                  PoissonRegressor, RANSACRegressor, Ridge,
...
def evaluate_model(model, model_type, x_test, y_pred, y_true, get_score_only, **kwargs):
    ...
    if y_pred.ndim > 1:
        if y_true.shape[1] > 1 or y_pred.shape[1] > 1:
            logger.info(
                f"Multitarget {model_type} Evaluation: calculating {model_type} score"
            )
            return {f"{model_type} score": model.score(x_test, y_true)}
    ...
    if metrics:
        for metric in metrics:
            ...
            if type_of_target(y_true) in (
                "multiclass-multioutput",
                "multilabel-indicator",
                "multiclass",
            ) and metric.__name__ in (
                "precision_score",
                "accuracy_score",
                "recall_score",
                "f1_score",
                ...
                if metric.__name__ == "accuracy_score":
                    ...
                    eval_res[metric.__name__] = metric(
                        y_pred=y_pred, y_true=y_true, average="micro"
                    )

            else:
                eval_res[metric.__name__] = metric(
                    y_pred=y_pred, y_true=y_true, **kwargs
                )
