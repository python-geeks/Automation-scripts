...
class FileSearch:
    ...
    def __perform_search(
        self, search_term, start_dir, is_folder, disable_case, show_bar
    ):
        results = []
    ...
    def search(
        self, search_term, start_dir, is_folder=False, disable_case=False, show_bar=True
    ):
        self.file_count = 0
        ...
        result = self.__perform_search(
            search_term, start_dir, is_folder, disable_case, show_bar
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "search-term", metavar="term", type=str, help="Filename/Word to search for"
    )
    parser.add_argument(
        "start-dir", metavar="dir", type=str, help="Directory where you want to search"
    )
    ...
    parser.add_argument(
        "-nb", "--no-bar", action="store_false", help="Not show progress bar"
    )
