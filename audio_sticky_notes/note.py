...
def rec(duration):
    ...
    return sf.write(
        "assets\\recordings\\AudioNote{}.flac".format(str(sys.argv[1])), recording, fs
    )
...
tkinter.Label(master, text=" Record for: (sec) ").grid(
    row=0, sticky=tkinter.W, rowspan=5, padx=15
)
...
tkinter.Label(master, text=" Playback: ").grid(
    row=5, sticky=tkinter.W, rowspan=5, padx=15
)
