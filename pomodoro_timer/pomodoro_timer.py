...
def user_start():
    ...
    return threading.Thread(target=start_timer, args=[start_timer_params]).start()
...
# Title
title_label = Label(root, text="Focus...", font=("MV Boli", 20, BOLD), bg="Salmon")
title_label.pack()

# Cycle Number
cycle_label = Label(root, font=("Consolas", 20, BOLD), bg="Salmon")

# Pomodoro Timer
timer_label = Label(
    root, text="Timer here", bg="black", fg="green", font=("Consolas", 32), width=10
)
...
input_label_work = Label(
    input_frame, text="Work (in mins)", bg="salmon", font="Serif 10"
)
input_label_break = Label(
    input_frame, text="Break (in mins)", bg="salmon", font="Serif 10"
)

input_label_work.grid(row=2, column=0)
input_label_break.grid(row=2, column=1)

user_input_work = Entry(input_frame, font=("Sans Serif", 12), relief=FLAT, width=10)
user_input_break = Entry(input_frame, font=("Sans Serif", 12), relief=FLAT, width=10)

user_input_work.grid(row=1, column=0)
user_input_break.grid(row=1, column=1, padx=10)

user_input_start = Button(
    input_frame, text="Start", font=("Sans Serif", 8), relief=FLAT, command=user_start
)
user_input_start.grid(row=1, column=2)

# Date time label
date_label = Label(root, text="Datetime:", font=("Consolas", 14), bg="salmon")
