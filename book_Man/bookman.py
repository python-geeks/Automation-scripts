...
class Ui_MainWindow(object):
    def setupUi(self,  MainWindow):
        ...
        for item in books:
            newitem=QtWidgets.QListWidgetItem(f"Name: {item[0]}\nPath: {item[1]}\nTags: {', '.join(item[2])}\nNotes: {item[3]}\n")
        ...
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("Icons/search.webp"),  QtGui.QIcon.Normal,  QtGui.QIcon.Off)
    ...
    def search_books(self):
        ...
        field = [self.radioButton_1.isChecked(), self.radioButton_2.isChecked(), self.radioButton_3.isChecked(), self.radioButton_4.isChecked()]
    ...
    def add_books(self):
        ...
        if s==1:
            if ui.data['path'] not in [i[1] for i in books]:
                books.append([ui.data['name'], ui.data['path'], ui.data['tags'], ui.data['notes']])
                item=books[-1]
                # self.list_1.addItem(f"Name: {item[0]}\nPath: {item[1]}\nTags: {', '.join(item[2])}\nNotes: {item[3]}\n")
                newitem=QtWidgets.QListWidgetItem(f"Name: {item[0]}\nPath: {item[1]}\nTags: {', '.join(item[2])}\nNotes: {item[3]}\n")
                font = QtGui.QFont('Times', 12)
                font.setBold(True)
                font.setWeight(50)
                newitem.setFont(font)
                self.list_1.addItem(newitem)
                add_book(conn, Book(ui.data['name'], ui.data['path'], ', '.join(item[2]), ui.data['notes']))
    ...
    def open_properties(self, item):
        ...
        Dialog2 = QtWidgets.QDialog()
        ui = Ui_Dialog()
        ui.setupUi(Dialog2, path=final[1], name=final[0], tags=final[2], notes=final[3])
        Dialog2.show()
        s = Dialog2.exec_()
        if s==1:
            if ui.data['delete']==True:
                ...
                edit_book(conn, 'tags', ', '.join(books[index][2]), books[index][1])
