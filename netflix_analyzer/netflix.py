...
def clean_data(user_name: str, data_frame: object) -> object:
    ...
    data[["TV Show", "Season", "Episode"]] = data["Title"].str.split(
        ":", expand=True, n=2
    )
    data["Content Type"] = data["Season"].apply(
        lambda x: "Movie" if x is None else "TV Show"
    )
    ...
    data["Start Time"] = data["Date"].apply(
...
def average_time_spent_per_day(data: object) -> None:
    ...
    font = {"family": "serif", "color": "#004466", "weight": "normal", "size": 18}

    plt.figure(figsize=(20, 15))
    ax = sns.barplot(
        x=avg_second_per_day.index,
        y=avg_second_per_day.values,
        palette=("crest_r"),
        order=day_of_week,
    )
    ax.set_ylabel(
        "Viewing Duration (in Seconds)", fontdict={"size": 14, "family": "serif"}
    )
...
def top_watched_TV_show(data: object) -> object:
    ...
    most_watched = (
        order_tv_shows.mean().reset_index().sort_values(by="Duration", ascending=False)
    )
    ...
    ax = sns.barplot(
        y=most_watched["TV Show"][:10],
        x=most_watched["Duration"][:10],
        palette=("winter"),
    )
...
def binge_watch(tv_shows: object) -> None:
    ...
    binge = (
        ...
        .count()
        .reset_index()
        .sort_values(by="Episode", ascending=False)
    )
    binge = binge[binge["Episode"] > 5]
    top_10_binge = (
        binge.groupby("TV Show")["Episode"]
        .sum()
        .reset_index()
        .sort_values(by="Episode", ascending=False)
    )
    ...
    plt.figure(figsize=(22, 12))
    ax = sns.barplot(
        y=top_10_binge["TV Show"][:10],
        x=top_10_binge["Episode"][:10],
        orient="h",
        ci="None",
    )
    ax.set_ylabel("TV Series", fontdict={"size": 14, "family": "serif"})
