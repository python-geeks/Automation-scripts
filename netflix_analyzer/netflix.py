...
def clean_data(user_name: str, data_frame: object) -> object:
    ...
    data['Content Type'] = data['Season'].apply(lambda x : 'Movie' if x == None else 'TV Show')
    ...
    data = data[['Profile Name', 'Date', 'date_of_month', 'day_of_week', 'day_name', 'Month', 'Year', 'Duration', 'Title', 'TV Show', 'Season', 'Episode', 'Content Type', 'Device Type']]
    ...
    data['Duration (Seconds)'] = data['Duration'].str.split(':').apply(lambda x: int(x[0])*3600 + int(x[1])*60 + int(x[2]))
...
def average_time_spent_per_day(data: object) -> None:
    ...
    avg_seconds_per_day = data.groupby('day_name')['Duration (Seconds)'].mean().sort_values()
    ...
    ax = sns.barplot(x = avg_seconds_per_day.index, y = avg_seconds_per_day.values, palette=("crest_r"), ci = None, order=day_of_week)
...
def top_watched_TV_show(data: object) -> object:
    ...
    ax = sns.barplot(y = most_watched_tv_series['TV Show'][:10], x = most_watched_tv_series['Duration (Seconds)'][:10], orient = 'h', palette=("winter"), ci = None)
...
def average_time_spent_per_month(data: object) -> None:
    ...
    ax = sns.boxplot(x = data['Month'], y = data['Duration (Seconds)'])
