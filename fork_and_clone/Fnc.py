...
def fork():
    ...
    accOrOrg = input("Fork to your profile or an organization (type org for organization, acc for profile): ")
...
def clone(repo):
    ...
    location = input("Path for cloning repo into (default is current directory): ")
    try:    
        if len(location.strip()) != 0:
            arr = repo.split("/")
            subprocess.run("git clone https://github.com/" + repo + ".git " + location + "\\" + arr[0] + "\\" + arr[1] , shell=True, check=True)
        else:
            subprocess.run("git clone https://github.com/" + repo + ".git" , shell=True, check=True)    
    except Exception as e:
        print("ERROR: " + str(e))
        fnc = input("To start from the forking type fork, to start from cloning type clone, to exit press ctrl-c: ").lower()
        if fnc == "fork": fork()
        elif fnc == "clone": clone(repo)
        else: 
