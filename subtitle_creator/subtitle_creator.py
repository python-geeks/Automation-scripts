...
def write_text(text_chunks,original_video,font_size = 15,output):
    txt_clips = []
    for i in range(len(text_chunks)):
        txt_clips.append( TextClip(text_chunks[i],fontsize=font_size,color="yellow").set_position('bottom').set_duration(3).set_start(i*3))
...
if __name__ == "__main__":
    ...
    if (len(sys.argv)) > 3:
        write_text(get_text(),VideoFileClip(sys.argv[1]),sys.argv[2],sys.argv[3])
    else:
        write_text(get_text(),VideoFileClip(sys.argv[1]),output = sys.argv[2])
